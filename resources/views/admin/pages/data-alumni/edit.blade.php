<div class="modal fade" id="edit{{ $data->id }}" tabindex="-1" aria-hidden="true" style="display: none;">
    <div class="modal-dialog" role="document">
        <form action="/admin-pplg/data-alumni/{{ $data->id }}" method="post" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel1">Ubah Alumni</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="row">
                        <div class="col mb-3">
                            <label for="nameBasic" class="form-label">Nama Alumni</label>
                    ...
                    <div class="row">
                        <div class="col mb-3">
                            <label for="nameBasic" class="form-label">Profesi</label>
                            <input type="text" id="nameBasic" class="form-control" value="{{ $data->profesi }}"
                                name="profesi">
                        </div>
                        @error('profesi')
                            <p class="text-danger text-sm">{{ $message }}</p>
                    ...
                    <div class="row">
                        <div class="col mb-3">
                            <label for="nameBasic" class="form-label">Words</label>
                            <textarea id="nameBasic" class="form-control" name="words">{{ $data->words }}</textarea>
                        </div>
                        @error('words')
                            <p class="text-danger text-sm">{{ $message }}</p>
                        @enderror
